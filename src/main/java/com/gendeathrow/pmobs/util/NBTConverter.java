...
public class NBTConverter
{
		...
		for(Entry<String,JsonElement> entry : jObj.entrySet())
		{
			...
			} else
			{
				...
				} catch(Exception e)
				{
					if(tags.hasKey(key))
					{
						RaidersMain.logger.log(Level.WARN, "JSON/NBT formatting conflict on key '" + key + "'. Skipping...");
						continue;
					}
				}
	...
	private static NBTBase JSONtoNBT_Element(JsonElement jObj, byte id, boolean format)
	{
		...
		{
			RaidersMain.logger.log(Level.ERROR, "An error occured while parsing JsonElement to NBTBase (" + tagID + "):", e);
		}
		
		RaidersMain.logger.log(Level.WARN, "Unknown NBT representation for " + jObj.toString() + " (ID: " + tagID + ")");
		return new NBTTagString();
	}
